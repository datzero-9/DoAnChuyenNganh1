      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View className="flex-1">
          <View className="flex-row items-center px-4 py-3 border-b border-gray-200 bg-white">
            <Image source={{ uri: "https://thethaovanhoa.mediacdn.vn/372676912336973824/2024/6/19/lisa-blackpink-17187640024741002095355.jpg" }} className="w-10 h-10 rounded-full mr-3" />
            <View className="flex-1">
              <Text className="text-base font-semibold">Lisa</Text>
          ...
          >
            {messages.map((msg) => (
              <View key={msg.id} className={`mb-4 ${msg.fromMe ? 'items-end' : 'items-start'}`}>
                <View className={`max-w-[80%] rounded-2xl px-4 py-2 ${msg.fromMe ? 'bg-indigo-500' : 'bg-gray-100'}`}>
